...
## Added by Chris
def _handle_imgs(imgs, batch_size, sess):
    imgs = imgs.astype(np.float32)
    m, s = calculate_activation_statistics(imgs, sess, batch_size=batch_size)
    return m, s

def calculate_fid_given_imgs(imgs1, imgs2, batch_size, inception_path=None):    
    ''' Calculates the FID of two paths. '''
    inception_path = check_or_download_inception(inception_path)
    g = create_inception_graph(str(inception_path))
    print("TYPE", type(g))
    with tf.Session(graph=g) as sess:
        sess.run(tf.global_variables_initializer())
        m1, s1 = _handle_imgs(imgs1, batch_size, sess)
        if type(imgs2) == str and imgs2.endswith(".npz"):
            f = np.load(imgs2)
            m2, s2 = f['mean'], f['cov']
            f.close()
        else:
            m2, s2 = _handle_imgs(imgs2, batch_size, sess)
        fid_value = calculate_frechet_distance(m1, s1, m2, s2)
        return fid_value
